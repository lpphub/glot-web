...
import { postRole } from '@/service/api';
...
type Model = Pick<Api.SystemManage.Role, 'id' | 'name' | 'code' | 'desc' | 'status'>;
...
type RuleKey = Exclude<keyof Model, 'id' | 'desc'>;
...
async function handleSubmit() {
  await validate();
  const { error } = await postRole(model as Api.SystemManage.Role);
  if (!error) {
